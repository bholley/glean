...
GLEAN_PYENV := $(shell python3 -c "import sys; print('.venv' + '.'.join(str(x) for x in sys.version_info[:2]))")
...
build-python: setup-python ## Build the Python bindings
	PATH=$(PWD)/bin:$(PATH) \
	VIRTUAL_ENV=$(PWD)/$(GLEAN_PYENV) \
		$(GLEAN_PYENV)/maturin develop

build-python-wheel: setup-python  ## Build a Python wheel
	PATH=$(PWD)/bin:$(PATH) \
	VIRTUAL_ENV=$(PWD)/$(GLEAN_PYENV) \
		$(GLEAN_PYENV)/maturin build --release $(MATURIN_FLAG)

build-python-sdist: setup-python ## Build a Python source distribution
	PATH=$(PWD)/bin:$(PATH) \
	VIRTUAL_ENV=$(PWD)/$(GLEAN_PYENV) \
		$(GLEAN_PYENV)/maturin build --release --sdist $(MATURIN_FLAG)
